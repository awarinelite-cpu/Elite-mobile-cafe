...
import { Plus, Upload, Trash2, Edit2, X, CheckCircle, Send, DollarSign } from 'lucide-react';
...
export default function AdminPage() {
  ...
  // Topic modal
  const [topicModal, setTopicModal] = useState(null); // null | 'new' | topic obj
  ...
  const openTopicModal = (topic = null) => {
    setTopicForm(topic ? { title: topic.title, category: topic.category, description: topic.description || '', level: topic.level || '', estimatedPrice: topic.estimatedPrice || '' } : { title: '', category: '', description: '', level: '', estimatedPrice: '' });
  ...
  const saveTopic = async () => {
    ...
    try {
      if (topicModal === 'new') {
        const doc = await createTopic(topicForm);
        setTopics(t => [{ id: doc.id, ...topicForm, active: true }, ...t]);
                ...
                <div key={order.id} className="card" style={{ padding: '18px 22px' }}>
                  ...
                  <div style={{ display: 'flex', gap: 10, flexWrap: 'wrap', alignItems: 'center' }}>
                    ...
                    {order.status === 'negotiating' && (
                      <button className="btn btn-ghost btn-sm" onClick={() => updateStatus(order.id, 'advance_due')}>
                        → Advance Due
                      </button>
                    )}
                    ...
                    {order.advancePaid && !order.balancePaid && order.deliverableUrl && order.status !== 'awaiting_final_payment' && (
                      <button className="btn btn-ghost btn-sm" onClick={() => updateStatus(order.id, 'awaiting_final_payment')}>
                    ...
                    {order.agreedPrice && (
                      <span style={{ marginLeft: 'auto', fontFamily: 'var(--font-mono)', fontSize: '0.82rem', display: 'flex', gap: 10 }}>
                        {order.advancePaid ? <span style={{ color: 'var(--sage, #2a5c45)' }}>✓ Advance</span> : <span style={{ color: 'var(--muted)' }}>○ Advance</span>}
                        {order.balancePaid ? <span style={{ color: 'var(--sage, #2a5c45)' }}>✓ Balance</span> : <span style={{ color: 'var(--muted)' }}>○ Balance</span>}
                      </span>
                    )}
                  </div>
                </div>
